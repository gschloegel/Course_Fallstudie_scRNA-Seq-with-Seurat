...
In this document only the differences to the original workflow are described. A more detailed description of the workflow is found in the tutorial description.

## Preperation

The workflow of the Seurat tutorial is adapted to the dataset
...
path is changed to the folder with the new data.
...
In the new dataset the mitochondrial genes start with "mt-". The lower case is not recognized by the original pattern. It was adapted to find the desired genes in the new dataset.
```{r}
# The [[ operator can add columns to object metadata. This is a great place to stash QC stats
pbmc[["percent.mt"]] <- PercentageFeatureSet(pbmc, pattern = "^MT-|^mt-") # changed pattern
...
the used data has a higher number of features. Therefore the filter was redefined according to the plots above.
```{r}
pbmc <- subset(pbmc, subset = nFeature_RNA > 1000 & nFeature_RNA < 7000 & percent.mt < 10) # adapted values according to plots
...
As in the Tutorial we use the heat maps to select the PCs containing the most relevant information for our dataset. We observe that the separation is no optimal and that PC1 has the biggest impact.
```{r}
...
```
### Select PCs
The Jack Straw Plot is not conclusive. The ellbow plot suggest that 15 PCs could be a good choice. We will later try to differnt amount of PCs to check if the rusults are robust to these changes.
```{r}
used_dims = 15
```
...
We search for the 2 genes with the biggest fold change for each cluster. Later we use them to mark the clusters.
